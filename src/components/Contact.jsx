    ...
    <AnimatePresence>
       { 
          ...
          <section className='h-full w-full absolute'>
              ...
              <motion.article initial={{y:100,opacity:0}} animate={{y:0,opacity:1}} transition={{duration:0.5}} exit={{y:100,opacity:0}} className="absolute top-[84%] left-[27%] p-[10px] flex gap-[50px] rounded-[10px] bg-[rgba(100,200,255,0.2)] z-30 shadow-[0_0_10px_rgb(100,200,255)]">
              {
                contact.map((items,index)=>(
                  <a  key={index} href={items.href} className="flex justify-center items-center text-blue-200 gap-[10px]">
                     <p>{items.icon}</p>
